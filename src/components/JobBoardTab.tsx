      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs space-y-3">
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl border text-xs font-bold flex items-center gap-1.5 transition-colors ${showFilters ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-700 border-slate-200 hover:border-slate-300"}`}
          >
        ...
        {showFilters && (
          <div className="flex flex-wrap gap-3 pt-2 border-t border-slate-100">
              ...
              <div className="flex flex-wrap gap-1.5">
                {CATEGORIES.map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)}
                    className={`px-2.5 py-1 rounded-lg text-[11px] font-bold border transition-all ${selectedCategory === cat ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:border-indigo-300"}`}>
                    {cat}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Work Style</label>
              <div className="flex flex-wrap gap-1.5">
                {REMOTE_OPTIONS.map(opt => (
                  <button key={opt} onClick={() => setSelectedRemote(opt)}
                    className={`px-2.5 py-1 rounded-lg text-[11px] font-bold border transition-all ${selectedRemote === opt ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-200 hover:border-slate-400"}`}>
                    {opt}
                  </button>
                ))}
              </div>
            ...
            <div className="ml-auto">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Sort By</label>
              <div className="flex gap-1.5">
                {(["match", "recent"] as const).map(s => (
                  <button key={s} onClick={() => setSortBy(s)}
                    className={`px-2.5 py-1 rounded-lg text-[11px] font-bold border transition-all capitalize ${sortBy === s ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-slate-600 border-slate-200 hover:border-slate-400"}`}>
        ...
        <div className="flex gap-1.5 overflow-x-auto no-scrollbar">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setSelectedCategory(cat)}
              className={`shrink-0 px-3 py-1.5 rounded-full text-[10px] font-bold border transition-all ${selectedCategory === cat ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:border-indigo-300"}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-7 space-y-3">
          {filtered.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
              ...
              <h3 className="font-bold text-slate-700">No roles match your filters</h3>
              ...
              <button onClick={() => { setSearch(""); setSelectedCategory("All"); setSelectedRemote("All"); }}
                className="mt-4 text-xs font-bold text-indigo-600 hover:underline">Clear all filters</button>
            </div>
          ) : (
            filtered.map(job => (
              <div
                ...
                className={`bg-white rounded-2xl border transition-all cursor-pointer hover:shadow-md group ${selectedJob?.id === job.id ? "border-indigo-400 ring-2 ring-indigo-100" : "border-slate-200 hover:border-indigo-200"}`}
              >
                  ...
                  <div className="flex items-start gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <div>
                          <h3 className="text-sm font-black text-slate-900 group-hover:text-indigo-600 transition-colors leading-tight">{job.title}</h3>
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1 text-xs text-slate-500 font-medium">
                            ...
                            <span className="flex items-center gap-1 font-bold text-slate-700"><DollarSign className="w-3.5 h-3.5" />{job.salary}</span>
                          </div>
                        </div>
                        <div className={`shrink-0 px-2.5 py-1 rounded-full text-[10px] font-black border ${job.match >= 90 ? "bg-emerald-50 text-emerald-700 border-emerald-200" : job.match >= 80 ? "bg-indigo-50 text-indigo-700 border-indigo-200" : "bg-slate-100 text-slate-600 border-slate-200"}`}>
                          {job.match}% Match
                        </div>
                      </div>
                      <div className="flex flex-wrap items-center gap-2 mt-3">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold border ${remoteColor[job.remote] || "bg-slate-50 text-slate-600 border-slate-200"}`}>
                          {job.remote}
                        </span>
                        {job.tags.slice(0, 3).map(tag => (
                          <span key={tag} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md text-[10px] font-bold">{tag}</span>
                        ))}
                        {job.tags.length > 3 && <span className="text-[10px] text-slate-400">+{job.tags.length - 3}</span>}
                ...
                <div className="px-5 py-3 bg-slate-50 rounded-b-2xl border-t border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-1.5 rounded-lg hover:bg-white border border-transparent hover:border-slate-200 transition-all"
                    >
                      {bookmarks.includes(job.id)
                        ? <BookmarkCheck className="w-4 h-4 text-indigo-600" />
                        : <Bookmark className="w-4 h-4 text-slate-400" />}
                    </button>
                    <span className="text-[10px] text-slate-500 font-bold">{job.category}</span>
                  </div>
                  <button
        ...
        <div className="lg:col-span-5">
          {selectedJob ? (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm sticky top-20 overflow-hidden">
              <div className="p-5 border-b border-slate-100">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="font-black text-slate-900 text-sm leading-tight">{selectedJob.title}</h2>
                    <p className="text-xs text-slate-500 mt-0.5">{selectedJob.company} · {selectedJob.location}</p>
                  </div>
                  <button onClick={() => setSelectedJob(null)} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  ...
                  <span className="px-2.5 py-1 rounded-full text-[10px] font-bold border bg-slate-100 text-slate-700 border-slate-200 flex items-center gap-1">
                    <DollarSign className="w-3 h-3" />{selectedJob.salary}
                  </span>
                  <span className="px-2.5 py-1 rounded-full text-[10px] font-bold border bg-amber-50 text-amber-700 border-amber-100 flex items-center gap-1">
                    <Clock className="w-3 h-3" />{selectedJob.postedAt}
                  </span>
                  <span className={`ml-auto px-2.5 py-1 rounded-full text-[10px] font-black border ${selectedJob.match >= 90 ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-indigo-50 text-indigo-700 border-indigo-200"}`}>
              ...
              <div className="p-5 space-y-5">
                <div>
                  <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider mb-2">About the Role</h3>
                  <p className="text-xs text-slate-600 leading-relaxed">{selectedJob.description}</p>
                </div>

                <div>
                  <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider mb-2">Required Skills</h3>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedJob.tags.map(tag => (
                      <span key={tag} className="px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-700 border border-indigo-100 text-[11px] font-bold flex items-center gap-1">
                        <CheckCircle2 className="w-3 h-3 text-indigo-500" />{tag}
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-black text-indigo-900">Match Analysis</span>
                  </div>
                  <div className="w-full h-2 bg-indigo-200 rounded-full overflow-hidden mb-2">
                    <div className="h-full bg-indigo-600 rounded-full transition-all" style={{ width: `${selectedJob.match}%` }} />
                  </div>
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-xl font-bold text-xs border transition-all flex items-center justify-center gap-1.5 ${bookmarks.includes(selectedJob.id) ? "bg-indigo-50 text-indigo-700 border-indigo-200" : "bg-white text-slate-700 border-slate-200 hover:border-slate-300"}`}
                  >
            ...
            <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-10 text-center sticky top-20">
              <Briefcase className="w-12 h-12 text-slate-200 mx-auto mb-3" />
              <h3 className="font-bold text-slate-700 text-sm">Select a Role</h3>
              <p className="text-xs text-slate-400 mt-1">Click any job card to view full details and your match analysis.</p>
            </div>
          )}
        </div>
      </div>
