...
export function AiToolsTab() {
  ...
  const [suggestions, setSuggestions] = useState<{ original: string; suggestions: string[] } | null>(null);
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-xs space-y-5">
        <h2 className="text-sm font-black text-slate-900 flex items-center gap-2 border-b pb-3">
          <Wand2 className="w-4 h-4 text-purple-600" /> Interactive Bullet Enhancer
        </h2>

        <form onSubmit={handleEnhance} className="space-y-4">
          <div>
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1.5">Your Draft Bullet</label>
            <textarea
              rows={2}
              value={inputText}
              onChange={e => setInputText(e.target.value)}
              placeholder="e.g. Built a website for campus club members to track attendance"
              className="w-full px-3 py-2.5 text-xs rounded-xl border border-slate-200 focus:ring-2 focus:ring-purple-500 outline-none resize-none transition-all"
            />
          </div>

          <div>
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1.5">Target Domain</label>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {DOMAINS.map(d => (
                <button key={d.id} type="button" onClick={() => setDomain(d.id)}
                  className={`py-2.5 px-3 rounded-xl border text-[11px] font-bold transition-all ${domain === d.id ? "bg-purple-600 text-white border-purple-600 shadow-sm" : "bg-white text-slate-600 border-slate-200 hover:border-purple-300"}`}>
                  {d.label}
                </button>
              ))}
            </div>
        ...
        {suggestions && (
          <div className="pt-4 border-t border-slate-100 space-y-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider">AI-Generated Power Variations</h3>
            </div>

            {/* Original */}
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Original Draft</p>
              <p className="text-xs text-slate-600 italic">{suggestions.original}</p>
            </div>

            {/* Enhanced Versions */}
            <div className="space-y-2">
              {suggestions.suggestions.map((sug, i) => (
                <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-purple-50/50 border border-purple-100 group">
                  <div className="w-5 h-5 shrink-0 rounded-full bg-purple-100 flex items-center justify-center text-[10px] font-black text-purple-700 mt-0.5">{i + 1}</div>
                  <p className="flex-1 text-xs text-slate-800 leading-relaxed">{sug}</p>
                  <button onClick={() => copyText(sug, i)}
                    className="shrink-0 p-1.5 rounded-lg bg-white border border-purple-200 text-purple-700 hover:bg-purple-100 transition-colors opacity-0 group-hover:opacity-100">
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-xs space-y-5">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h2 className="text-sm font-black text-slate-900 flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-indigo-600" /> 150+ Power Verbs Dictionary
            </h2>
            <p className="text-[11px] text-slate-500 mt-0.5">Click any verb to copy instantly to clipboard.</p>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            {activeCategory && (
              <button onClick={() => setActiveCategory(null)} className="text-xs font-bold text-indigo-600 hover:underline whitespace-nowrap">Clear</button>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {ACTION_VERB_CATEGORIES.map(cat => (
            <button key={cat.category} onClick={() => setActiveCategory(activeCategory === cat.category ? null : cat.category)}
              className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-all ${activeCategory === cat.category ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:border-indigo-300"}`}>
              {cat.category}
            </button>
          ))}
        </div>
        ...
        {copiedVerb && (
          <div className="p-2.5 bg-emerald-50 text-emerald-800 text-xs font-bold rounded-xl border border-emerald-200 text-center animate-fade-in-up">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredCategories.map((cat, idx) => (
            <div key={idx} className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2.5">
              <h3 className="text-[10px] font-black text-slate-700 uppercase tracking-wider">{cat.category}</h3>
              <div className="flex flex-wrap gap-1.5">
